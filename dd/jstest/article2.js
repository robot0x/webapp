...
function abcdefg(data) {
    console.log("callback invoked!");
    var hotList = $(".hot-list"),url,match,imgUrl;
...
//http://api.diaox2.com/v1/stat/all?&cb=jQuery111308396169231273234_1443183199217&{%22a%22:1,%20%22b%22:2}
    ...
    success: abcdefg,   //jquery will map "mycb" ==> success function
    error:function(data){
...
var $method = $(".method"),
    ...
    $methodSpan = $method.find("span"),par;
$methodSpan.hover(function(){
        par = this.parentNode;
        if(!$.contains(par,par.querySelector("."+tipsClass))){
        $(par).css({"position":"relative","z-index":0}).append("<span class='"+tipsClass+"' style='position:absolute;top:-72%;left:25px;width:auto;font-size:14px;z-index:-1;'>"+this.innerHTML+"</span>");
        }
        },function(){
        par = this.parentNode;
