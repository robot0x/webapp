...
function build_button(data) {
    ...
    for(var i = 0; i < data.res.user_data.awards.length; i++) {
        ...
        a_arr.push(item.award_info.name);
    }
    g_myprize = a_arr.join(",");
    if(data.res.user_data.awards.length != 0)
        isWinning = true;
    return true;
}
            ...
            if(data.res.ctype == 0) {
                ...
                showWinningDialog(winningDialog,'恭喜您中奖了','您的奖品是：<span>' + g_myprize + '</span>',function(e){
                    ...
                    ajax({
                        ...
                        error:function(msg){
                            console.log(msg); console.log('添加地址失败');
                        }
                    });}, function(e){
                    winningDialog.style.display = 'none';
                });
...
function choujiang(from) {
    ...
    jd["signature"] = md5(jd["signature"]);
    if(from && from == ".wb")
        js["user_action"] = {type:"weibo"};
    if(from && from == ".wx")
        js["user_action"] = {type:"wechat_timeline"}
    if(from == undefined) {
            ...
            if(data.res.ctype == 0) {
                ...
                showWinningDialog(winningDialog,'恭喜您中奖了','您的奖品是：<span>' + g_myprize + '</span>',function(e){
                    ...
                    ajax({
                        ...
                        data: JSON.stringify(jd),
                        dataType: "json",
                        url: "http://api.diaox2.com/v2/usr/address",
                        success:function(data){
                            winningDialog.style.display = 'none';
                            },
                            error:function(msg){
...
viewMyPrize.onclick = function(){
    if(isWinning){ //中奖
        ...
        //g_myprize;
        var jd = {action:"get", activity_id:g_aid, uid:g_uid, did:g_did}
        jd["signature"] = "diaox2_query_coupon_" + g_uid + "_" + g_aid + "_" + g_did;
        jd["signature"] = md5(jd["signature"]);
