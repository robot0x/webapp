...
var succ_click = function(selector, extra) {
    ...
    return function(data) {
        ...
        console.log(data);
        // 这行代码要改成
        window.open(extra,'','resizable=no,alwaysRaised=yes,depended=yes,location=no,menubar=no');
...
//增加热门文章的函数簇
//默认在div#relcardlast前添加，且添加5个
//添加失败的话，默认加入一个刷新用的文章卡片
var relwidth = -1;
var relheight = -1;
var add_hot_success = function(selector, num, success_callback) {
    if(selector == undefined) {
        selector = "div#relcardlast";   //default selector
    }
    if(num == undefined || isNaN(num)) {
        num = 5;   //default num
    }
    if(relwidth == undefined || relwidth == -1 || relheight == undefined || relheight == -1) {
        var dummy = $('<div class="half"><div class="relcard"></div></div><div class="half"><div class="relcard"></div></div>').insertBefore(selector);
        //relwidth = dummy.find('relcard').width();
    ...
    return function(data) {
        ...
        while(i<num && j<data.length) {
            ...
            */
            ...
            //html_str += '</a></div></div>';
            html_str += '</div></div>';
            //html_str += '<span class="cardread" data-id="' + data[pos].serverid + '">阅读&nbsp;' + data[pos].total + '</span>\n</div>';
            //html_str += '\n</a></div><div class="clearfix"></div><hr class="sep">\n';
            /*[TODO]
                var allheight = Number.parseInt($('.relcontent').css('height')) - Number.parseInt($('.relcontent').css('padding-top')) - Number.parseInt($('.relcontent').css('padding-bottom'));
                var actualheight = $('.reltitle').height(); //TODO: locate this id
                var alltext = $('.reltitle').text();
                while(actualheight > allheight) {
                    alltext = alltext.slice(0, -1);
                    $('.reltitle').text(alltext);
                    actualheight = $('.reltitle').height();
                }
...
var add_hot_fail = function(selector, num, prefix, success_callback, fail_callback) {
    if(selector == undefined) {
        selector = "div#relcardlast";
    }
    return function (XMLHttpRequest, textStatus, errorThrown) {
        console.log('get hot article failed, with error:');
        console.log(errorThrown);
        //$('<div class="relcard reload"><div class="relpic"><img src="/cms/diaodiao/people/robot.jpg" /></div><div class="reltext"><span class="cardtitle">点击重新加载</span><span class="cardread">阅读&nbsp;999+</span></div></div><div class="clearfix"></div><hr class="sep">').insertBefore(selector);
        if($('.reload') == undefined || $('.reload').length == 0) {
            //TODO: check
            var html = '<div class="half"><div class="relcard reload"><div class="relimage" style="';
            html += 'style="width:' + relwidth + 'px; height:' + relheight + 'px;"';
            html += '><img src="/cms/diaodiao/people/robot.jpg" /></div><div class="relmask"></div><div class="relcontent"><table><tbody><tr><td class="reltitle">点击重新加载</td><td></td></tr></tbody></table></div></div></div>';
           $(html).insertBefore(selector);
           $('.reload').bind("click", function (){
               $('.reload').remove();
               initHot(selector, num, prefix, success_callback, fail_callback);
               return true;
            });
        }
...
function initHot(selector, num, prefix, success_callback, fail_callback)
{
    jQuery.support.cors = true;
    if(selector == undefined) {
        selector = "div#relcardlast";
    } else {
        //如果直接指定了不存在的selector，放弃执行
        if($(selector).length == 0 || (num && num <= 0))
            return;
    }
    if(num == undefined || isNaN(num)) {
        num = 5;
    ...
    $.ajax
        ...
        dataType: "JSON", //"html"
        url: targeturl,
        success: add_hot_success(selector, num, success_callback),
        timeout: api_ajax_timeout,
        error: add_hot_fail(selector, num, prefix, success_callback, fail_callback)
    });
    return true;
}
//刷新所有文章卡片的阅读数的函数
...
function gen_pic_info(selector, skiplist, dummypic) {
    ...
    if(dummypic == undefined || typeof(dummypic) != "object") {
        dummypic = {"/cms/diaodiao/assets/pixel.gif" : "1", "http://c.diaox2.com/cms/diaodiao/assets/pixel.gif": "1", "http://a.diaox2.com/cms/diaodiao/assets/pixel.gif": "1"};
    } else {
        dummypic["/cms/diaodiao/assets/pixel.gif"] = "1";
        dummypic["http://c.diaox2.com/cms/diaodiao/assets/pixel.gif"] = "1";
        dummypic["http://a.diaox2.com/cms/diaodiao/assets/pixel.gif"] = "1";
    }
    var jsondict = {"urls": []};
