...
$(function() {
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      if (result.special_query && result.special_query.query_type === "gift") {

        var tags = result.special_query.gift_tag_query_info.tags;
        /********** 工具方法区 start ************/
        // 去重方法
        ...
        function specSearchSuccess(data) {
            ...
            if (q !== '礼物') {
              // alert(1);
              triggerClickByQueryStatement(tags);
              return;
            } else {
              // 只搜礼物时，并没有排序
    ...
    TODO
  */
  function triggerClickByQueryStatement(tags) {
      tags.forEach(function(tag){
         handler.call(document.querySelector('[data-title="'+tag+'"]'));
      })
  }
  ...
  */
  ...
  function renderDOM(meta_infos) {
    ...
    var
      ...
      ks = keys(meta_infos).sort(function(a,b){return b - a}),
      count = i = 0,
      length = ks.length,
      dataPos = 1;
  ...
  */
  function updateDOM() {
    ...
    var
      ...
      gift_tag_index = data.gift_tag_index,
      ks = keys(needUpdateMeta).sort(function(a,b){return b - a}),
      count = i = 0,
      length = ks.length,
      dataPos = 1;
    while(i < length){
