...
$(function() {
  ...
  function renderDOMByIntersect() {
    ...
    if (interList && interList.length) {
      ...
      // console.log(meta_infos_obj);
      renderDOM(meta_infos_obj, interList);
    } else {
      ...
      if (curSelect.length) {
        ...
        renderDOM(meta_infos, interList);
      }
    }
  }
  /*
  ...
  */
  ...
  function intersect() {
    ...
    curSelect.each(function(index, item) {
        if (getDataAttribute(item, "group") === 'character') {
          characterArr.push(getDataAttribute(item, "title"));
        } else {
          notCharacterArr.push(getDataAttribute(item, "title"));
        }
      })
      // 求并集。先连接数组，再去重
    ...
    // metaResult.sort(function(a,b){return b - a});
    // console.log(metaResult);
    return metaResult.sort(function(a, b) {
      return b - a
    });
  }
  ...
  */
  var needUpdateMeta;
  function renderDOM(meta_infos, interList) {
    console.log(interList);
  ...
  function doupdate(cids) {
    ...
    $.ajax({
      ...
      jsonp: 'cb',
      // 取 cb5 ~ cb15 之间的随机数
      jsonpCallback: "cb" + Math.ceil(Math.random() * 5 + Math.random() * 15),
      cache: true,
      timeout: 2000,
  ...
  function get_stat() {
    ...
    (function(input) {
      (function() {
        ...
        if (pack.length > 0) {
          doupdate(pack);
          setTimeout(arguments.callee, 5000); //5秒后调用下一波
        } else {
          return true;
        }
      })()
  ...
  */
  ...
  闺蜜,女性朋友,女生,女性,女同学
  基友,男性朋友,男生,男性,男同学
  朋友,同事,普通朋友,盆友,同学,普通同学
  老板,领导,老师,导师,上司,上级
  */
  /**
   * [containStr 是否命中页面中的tag]
   * @param  {[String]} q    [页面搜索框中的关键字]
   * @param  {[String]} text [页面的tag]
   * @return {[Boolean]}     [页面的tag是否和搜索框中的关键字关联]
   *
   * 例如搜：
   *  男友礼物             -> 老公男友
   *  老公礼物             -> 老公男友
   ...
   *  清新礼物             -> 小清新
   *  萌萌礼物             -> 萌萌哒
   *  Geek/geek/GEEK/极客 -> Geek
   *  文艺礼物             -> 文艺范礼物
   *  潮流礼物/潮人礼物      -> 潮人
   *  添加自然语言支持：例如，给老公的生日礼物 -> 老公男友 & 生日
   */
  function targetTag(q, li) {
    if (!q || !li) return false;
    var keywords = getDataAttribute(li, "keywords");
    var title = getDataAttribute(li, "title");
    // 首先组装keywords数组
    if (keywords) {
  ...
  */
  function triggerClickByQueryStatement() {
    var queryStatement = q.split('+').join(''),
      liArr = document.querySelectorAll('.present-type li'),
      i = 0,
      length = liArr.length,
      item;
    /*
    ...
    */
    // 由forEach改成了for循环。好处是只要命中，就直接return
    // 如果用forEach的话，不管什么时候命中，都会遍历length次
    // 且省了一次nodeList转array的开销
    for (; i < length; i++) {
      item = liArr[i];
      if (getDataAttribute(item, "group") === "price") {
        continue;
      }
      // 命中tag
      if (targetTag(queryStatement, item)) {
    ...
    //   if(containStr(queryStatement,item.innerHTML)){
    //      handler.call(item);
    //   }
    //   // if(queryStatement.indexOf(item.innerHTML)!== -1){
    //   //   handler.call(item);
